...
# Configure the neuroglancer embedded server bind address before any Viewer is created.
# On a remote VM, set NEUROGLANCER_SERVER_HOST=0.0.0.0 (iframe URL will use socket.getfqdn())
# or set it to the VM's public IP/hostname directly.
# Set NEUROGLANCER_SERVER_PORT to a fixed port so it can be firewalled/proxied (default: random).
_NG_SERVER_HOST = os.environ.get("NEUROGLANCER_SERVER_HOST", "127.0.0.1")
_NG_SERVER_PORT = int(os.environ.get("NEUROGLANCER_SERVER_PORT", "0"))
import neuroglancer as _neuroglancer
_neuroglancer.set_server_bind_address(bind_address=_NG_SERVER_HOST, bind_port=_NG_SERVER_PORT)
logger.info(f"neuroglancer server bind: {_NG_SERVER_HOST}:{_NG_SERVER_PORT or '(random)'}")
