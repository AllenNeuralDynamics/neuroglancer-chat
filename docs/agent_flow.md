# Agent User Flow & Prompt Construction

## Overview

This document details how the LLM agent processes user queries, constructs prompts, manages conversation history, and executes tools in the Neuroglancer-Chat system.

---

## 🔄 Agent Execution Flow

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         USER INTERACTION FLOW                                │
└─────────────────────────────────────────────────────────────────────────────┘

User Types Message
      │
      ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  FRONTEND: panel_app.py                                                      │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │  respond() function                                                     │ │
│  │  • Receives user input                                                  │ │
│  │  • Chooses streaming or non-streaming path                             │ │
│  │  • Calls agent_call() or streams from /agent/chat/stream               │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────────────┘
      │
      ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  BACKEND: main.py                                                            │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │  /agent/chat or /agent/chat/stream endpoint                            │ │
│  │  • Receives: {"messages": [{"role": "user", "content": "..."}]}       │ │
│  │  • Calls: run_agent_chat() or run_agent_chat_stream()                 │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────────────┘
      │
      ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  AGENT LOOP: Iterative Tool Execution                                        │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │  run_agent_chat() - Main orchestration loop                            │ │
│  │                                                                          │ │
│  │  FOR EACH ITERATION (max 10):                                          │ │
│  │                                                                          │ │
│  │    1. BUILD PROMPT                                                      │ │
│  │       ├─ System Prompt (from llm.py)                                   │ │
│  │       ├─ Context (NG state summary + available data)                   │ │
│  │       ├─ Conversation History (user + assistant messages)              │ │
│  │       └─ Current User Message                                          │ │
│  │                                                                          │ │
│  │    2. CALL LLM (OpenAI API)                                            │ │
│  │       • Model: gpt-4-turbo or configured model                         │ │
│  │       • Temperature: 0 for deterministic responses                     │ │
│  │       • Tools: Available tool schemas from llm.py                      │ │
│  │                                                                          │ │
│  │    3. PARSE RESPONSE                                                    │ │
│  │       ├─ Text message? → Done, return to user                          │ │
│  │       ├─ Tool call(s)? → Execute tools                                 │ │
│  │       └─ Error? → Return error message                                 │ │
│  │                                                                          │ │
│  │    4. EXECUTE TOOLS (if present)                                       │ │
│  │       ├─ Call tool_dispatcher()                                        │ │
│  │       ├─ Track mutations (NG state changes)                            │ │
│  │       └─ Collect results                                               │ │
│  │                                                                          │ │
│  │    5. UPDATE HISTORY                                                    │ │
│  │       ├─ Add assistant message with tool calls                         │ │
│  │       └─ Add tool results as tool messages                             │ │
│  │                                                                          │ │
│  │    6. CONTINUE LOOP                                                     │ │
│  │       • LLM sees tool results and can:                                 │ │
│  │         - Make more tool calls                                         │ │
│  │         - Provide final answer to user                                 │ │
│  │                                                                          │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────────────┘
      │
      ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  RESPONSE AGGREGATION                                                        │
│  • Final assistant message                                                   │
│  • Mutation flag (if NG state changed)                                      │
│  • State link (if mutated)                                                   │
│  • Tool trace (execution history)                                           │
│  • Query data (if data_query executed)                                      │
│  • Plot data (if data_plot executed)                                        │
│  • NG views (if spatial query executed)                                     │
└─────────────────────────────────────────────────────────────────────────────┘
      │
      ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  FRONTEND RENDERING                                                          │
│  • Display assistant message                                                 │
│  • Render tables (Tabulator)                                                │
│  • Render plots (hvPlot)                                                     │
│  • Update Neuroglancer viewer (if mutated)                                  │
│  • Add to workspace                                                          │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 📝 Prompt Construction (Growing Context)

### Iteration 1: Initial User Query

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  PROMPT TO LLM (Iteration 1)                                                 │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  [SYSTEM PROMPT] (~2000 tokens)                                             │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │ You are a Neuroglancer assistant...                                   │  │
│  │                                                                        │  │
│  │ • Role description                                                     │  │
│  │ • Capabilities overview                                                │  │
│  │ • Tool usage guidelines                                                │  │
│  │ • Data manipulation examples (Polars expressions)                     │  │
│  │ • Aggregation patterns (group_by + agg)                               │  │
│  │ • Spatial data handling                                                │  │
│  │ • Plot generation guidelines                                           │  │
│  │ • Error handling expectations                                          │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
│                                                                              │
│  [CONTEXT] (~500-1000 tokens)                                               │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │ Current Neuroglancer State Summary:                                   │  │
│  │ • Layout: xy                                                           │  │
│  │ • Position: [1895.94, -2094.80, 689, 0]                              │  │
│  │ • Dimensions: x, y, z, t                                              │  │
│  │ • Layers: [CH_405, CH_488, CH_514]                                   │  │
│  │ • Annotation layers: []                                               │  │
│  │                                                                        │  │
│  │ Available Data Files:                                                  │  │
│  │ • cell_data_example.csv (1234 rows, 15 columns)                      │  │
│  │   - Columns: cell_id, x, y, z, volume, elongation, ...              │  │
│  │ • 767018_inh_cells.csv (5678 rows, 14 columns)                       │  │
│  │   - Columns: label, x_ccf, y_ccf, z_ccf, log_volume, ...            │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
│                                                                              │
│  [USER MESSAGE]                                                              │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │ {"role": "user", "content": "Show me cells with volume > 100"}       │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
│                                                                              │
│  [TOOLS AVAILABLE] (Function schemas)                                        │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │ • ng_set_view(center, zoom, orientation)                              │  │
│  │ • ng_set_lut(layer, vmin, vmax)                                       │  │
│  │ • ng_add_layer(name, type, source)                                    │  │
│  │ • data_query(file_id, expression, spatial_columns)                   │  │
│  │ • data_plot(file_id, plot_type, x, y, expression, ...)              │  │
│  │ • data_list_files()                                                   │  │
│  │ • ... (15+ tools total)                                               │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
│                                                                              │
│  TOTAL: ~2500-3500 tokens                                                   │
└─────────────────────────────────────────────────────────────────────────────┘
```

### LLM Response: Tool Call

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  LLM RESPONSE (Iteration 1)                                                  │
├─────────────────────────────────────────────────────────────────────────────┤
│  {                                                                           │
│    "role": "assistant",                                                      │
│    "content": null,                                                          │
│    "tool_calls": [                                                           │
│      {                                                                       │
│        "id": "call_abc123",                                                  │
│        "type": "function",                                                   │
│        "function": {                                                         │
│          "name": "data_query",                                               │
│          "arguments": {                                                      │
│            "file_id": "file_001",                                            │
│            "expression": "filter(pl.col('volume') > 100)",                  │
│            "spatial_columns": ["x", "y", "z"]                               │
│          }                                                                   │
│        }                                                                     │
│      }                                                                       │
│    ]                                                                         │
│  }                                                                           │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Backend Executes Tool

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  TOOL EXECUTION (Backend)                                                    │
├─────────────────────────────────────────────────────────────────────────────┤
│  tool_dispatcher() → execute_query()                                         │
│  • Load file_001 (cell_data_example.csv)                                    │
│  • Apply expression: filter(pl.col('volume') > 100)                         │
│  • Result: 456 rows × 15 columns                                            │
│  • Generate ng_views for spatial navigation                                 │
│  • Return structured data                                                    │
│                                                                              │
│  Tool Result:                                                                │
│  {                                                                           │
│    "ok": true,                                                               │
│    "rows": 456,                                                              │
│    "columns": ["cell_id", "x", "y", "z", "volume", ...],                   │
│    "data": {...},  # DataFrame as dict                                      │
│    "ng_views": [{row_index: 0, url: "..."}, ...],                          │
│    "expression": "filter(pl.col('volume') > 100)"                           │
│  }                                                                           │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

### Iteration 2: LLM Sees Tool Results

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  PROMPT TO LLM (Iteration 2) - GROWING CONTEXT                              │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  [SYSTEM PROMPT] (~2000 tokens)                                             │
│  • Same as Iteration 1                                                       │
│                                                                              │
│  [CONTEXT] (~500-1000 tokens)                                               │
│  • Same as Iteration 1                                                       │
│                                                                              │
│  [CONVERSATION HISTORY] (~1000-2000 tokens) ← NEW!                          │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │ {"role": "user", "content": "Show me cells with volume > 100"}       │  │
│  │                                                                        │  │
│  │ {"role": "assistant", "tool_calls": [                                │  │
│  │   {"function": {"name": "data_query", "arguments": {...}}}           │  │
│  │ ]}                                                                     │  │
│  │                                                                        │  │
│  │ {"role": "tool", "tool_call_id": "call_abc123",                      │  │
│  │  "content": "{\"ok\": true, \"rows\": 456, \"columns\": [...]}"}     │  │
│  │  # NOTE: Large data NOT sent to LLM! Only metadata                   │  │
│  │  # Actual data sent directly to frontend                             │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
│                                                                              │
│  [TOOLS AVAILABLE]                                                           │
│  • Same list as before                                                       │
│                                                                              │
│  TOTAL: ~3500-5500 tokens (growing with history)                            │
└─────────────────────────────────────────────────────────────────────────────┘
```

### LLM Response: Final Answer

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  LLM RESPONSE (Iteration 2)                                                  │
├─────────────────────────────────────────────────────────────────────────────┤
│  {                                                                           │
│    "role": "assistant",                                                      │
│    "content": "I found 456 cells with volume greater than 100. The         │
│                 results include spatial coordinates (x, y, z) for           │
│                 navigation in Neuroglancer. Click the View buttons to       │
│                 examine individual cells.",                                  │
│    "tool_calls": null                                                        │
│  }                                                                           │
│                                                                              │
│  → Loop terminates, returns response to user                                │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 🔁 Multi-Turn Conversation Example

### User Turn 1: "Load this state"

```
┌──────────────────────────┐
│  Prompt Size: ~2.5K tok  │  System + Context + "Load this state"
└──────────────────────────┘
              ↓
┌──────────────────────────┐
│  Tool: state_load()      │  → CURRENT_STATE updated
└──────────────────────────┘
              ↓
┌──────────────────────────┐
│  Prompt Size: ~4K tok    │  System + Context + History(2 msgs) + Tool Result
└──────────────────────────┘
              ↓
         "State loaded successfully"
```

### User Turn 2: "Show me the layers"

```
┌──────────────────────────┐
│  Prompt Size: ~5K tok    │  System + Context + History(3 msgs) + "Show layers"
└──────────────────────────┘
              ↓
┌──────────────────────────┐
│  No tool call            │  LLM describes layers from context
└──────────────────────────┘
              ↓
         "You have 3 layers: CH_405, CH_488, CH_514"
```

### User Turn 3: "Upload data and plot volume by cluster"

```
┌──────────────────────────┐
│  Prompt Size: ~6.5K tok  │  System + Context + History(5 msgs) + "Upload..."
└──────────────────────────┘
              ↓
┌──────────────────────────┐
│  Tool 1: data_list_files │  → Get available files
└──────────────────────────┘
              ↓
┌──────────────────────────┐
│  Prompt Size: ~8K tok    │  + Tool result
└──────────────────────────┘
              ↓
┌──────────────────────────┐
│  Tool 2: data_plot()     │  → Generate plot
└──────────────────────────┘
              ↓
┌──────────────────────────┐
│  Prompt Size: ~9.5K tok  │  + Tool result
└──────────────────────────┘
              ↓
         "Here's a bar plot of mean volume by cluster..."
```

### Conversation Growth Pattern

```
Turn 1:  2.5K tokens  (System + Context + 1 user msg)
Turn 2:  4.0K tokens  (+ 2 assistant msgs + 1 tool result)
Turn 3:  5.0K tokens  (+ 1 user msg + 1 assistant msg)
Turn 4:  6.5K tokens  (+ 1 user msg)
Turn 5:  8.0K tokens  (+ 1 assistant msg + 1 tool result)
Turn 6:  9.5K tokens  (+ 1 assistant msg + 1 tool result)
...
```

**Growth rate**: ~1.5-2K tokens per turn with tool execution
**Context window**: GPT-4-turbo supports 128K tokens
**Typical conversation**: 10-20 turns before hitting limits

---

## 🧩 System Prompt Components

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  SYSTEM PROMPT STRUCTURE (llm.py)                                            │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  1. ROLE DEFINITION                                                          │
│     "You are a Neuroglancer assistant for neuroimaging visualization..."    │
│                                                                              │
│  2. CAPABILITIES                                                             │
│     • Navigate 3D volumes                                                    │
│     • Adjust layer visibility and contrast                                   │
│     • Query and analyze cell data                                           │
│     • Generate plots and visualizations                                      │
│     • Manage multi-view states                                               │
│                                                                              │
│  3. DATA MANIPULATION GUIDELINES                                             │
│     ┌──────────────────────────────────────────────────────────────────┐   │
│     │ "When querying data, use Polars expressions:                     │   │
│     │                                                                   │   │
│     │ Filtering:                                                        │   │
│     │   filter(pl.col('volume') > 100)                                 │   │
│     │   filter((pl.col('x') > 0) & (pl.col('y') < 1000))             │   │
│     │                                                                   │   │
│     │ Aggregation:                                                      │   │
│     │   group_by('cluster_label').agg(pl.mean('volume'))              │   │
│     │   group_by('region').agg([                                       │   │
│     │     pl.mean('volume').alias('mean_vol'),                        │   │
│     │     pl.count().alias('count')                                    │   │
│     │   ])                                                              │   │
│     │                                                                   │   │
│     │ Sorting:                                                          │   │
│     │   sort('volume', descending=True)                                │   │
│     │                                                                   │   │
│     │ Column selection:                                                 │   │
│     │   select(['cell_id', 'x', 'y', 'z', 'volume'])                  │   │
│     └──────────────────────────────────────────────────────────────────┘   │
│                                                                              │
│  4. SPATIAL DATA HANDLING                                                    │
│     • Always preserve x, y, z columns                                        │
│     • Use spatial_columns parameter for navigation                           │
│     • Generate ng_views for clickable links                                  │
│                                                                              │
│  5. PLOT GENERATION EXAMPLES                                                 │
│     ┌──────────────────────────────────────────────────────────────────┐   │
│     │ Example 1: Simple bar plot                                       │   │
│     │   expression: "group_by('cluster').agg(pl.mean('volume'))"      │   │
│     │   x: 'cluster', y: 'volume'                                      │   │
│     │                                                                   │   │
│     │ Example 2: Scatter with filter                                   │   │
│     │   expression: "filter(pl.col('volume') > 50)"                   │   │
│     │   x: 'elongation', y: 'volume'                                   │   │
│     │                                                                   │   │
│     │ Example 3: Multi-step aggregation                                │   │
│     │   expression: "group_by('region').agg([                         │   │
│     │     pl.mean('log_volume').alias('mean_log_vol')                 │   │
│     │   ]).sort('mean_log_vol', descending=True)"                     │   │
│     └──────────────────────────────────────────────────────────────────┘   │
│                                                                              │
│  6. TOOL USAGE RULES                                                         │
│     • Call data_list_files() before querying unknown files                  │
│     • Use data_info() to inspect file structure                             │
│     • Don't send large data to user - use tables/plots                      │
│     • Always check tool results before responding                           │
│                                                                              │
│  7. ERROR HANDLING                                                           │
│     • Validate file_id exists                                                │
│     • Check column names before filtering                                    │
│     • Provide helpful error messages                                         │
│     • Suggest corrections for common mistakes                                │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 🛠️ Tool Schema Example

```json
{
  "type": "function",
  "function": {
    "name": "data_plot",
    "description": "Generate a plot from a data file. Supports scatter, line, bar, and heatmap plots with Polars expressions for data transformation.",
    "parameters": {
      "type": "object",
      "properties": {
        "file_id": {
          "type": "string",
          "description": "The file ID to plot from (use data_list_files to see available files)"
        },
        "plot_type": {
          "type": "string",
          "enum": ["scatter", "line", "bar", "heatmap"],
          "description": "Type of plot to generate"
        },
        "x": {
          "type": "string",
          "description": "Column name for x-axis"
        },
        "y": {
          "type": "string",
          "description": "Column name for y-axis (or list of columns)"
        },
        "expression": {
          "type": "string",
          "description": "Optional Polars expression to transform data before plotting. Examples: 'filter(pl.col(\"volume\") > 100)', 'group_by(\"cluster\").agg(pl.mean(\"volume\"))'"
        },
        "by": {
          "type": "string",
          "description": "Column to group by for colored/categorized plots. DO NOT use with aggregated bar plots."
        },
        "title": {
          "type": "string",
          "description": "Plot title"
        }
      },
      "required": ["file_id", "plot_type", "x", "y"]
    }
  }
}
```

---

## 📊 Memory & Context Management

### What Gets Saved in History?

```
✅ INCLUDED IN HISTORY:
• User messages (full text)
• Assistant messages (full text)
• Tool call requests (function name + arguments)
• Tool results (metadata only - no large data)
  - Success/failure status
  - Row counts, column names
  - Error messages
  - Summary statistics

❌ EXCLUDED FROM HISTORY:
• Large data payloads (sent directly to frontend)
• Plot HTML (generated client-side)
• Full dataframes (only metadata)
• Binary data (images, arrays)
```

### Context Pruning Strategy

Currently: **No automatic pruning** (relying on large context window)

Future considerations:
- Sliding window: Keep last N turns
- Importance-based: Keep tool calls, summarize text
- Compression: Summarize old conversations
- Checkpointing: Save state snapshots

---

## 🎯 Tool Execution Patterns

### Pattern 1: Single Tool → Answer

```
User: "Show layers"
  → No tool call (info in context)
  → Direct answer
```

### Pattern 2: Tool Chain

```
User: "Plot volume distribution"
  → Tool 1: data_list_files() 
  → Tool 2: data_plot()
  → Answer with plot
```

### Pattern 3: Parallel Tools

```
User: "Show file info and current state"
  → Tool 1: data_info()
  → Tool 2: ng_state_summary()
  → Synthesized answer
```

### Pattern 4: Error Recovery

```
User: "Plot invalid_column"
  → Tool: data_plot() → Error
  → Tool: data_info() → Get valid columns
  → Suggest correction to user
```

### Pattern 5: Multi-View Generation

```
User: "Show me top 5 cells"
  → Tool 1: data_query(limit=5)
  → Tool 2: ng_generate_multi_view()
  → NG state with 5 viewports
```

---

## 🔍 Debugging & Observability

### Tool Trace Structure

```json
{
  "traces": [
    {
      "timestamp": "2025-10-21T10:30:45Z",
      "conversation_id": "conv_123",
      "mutated": true,
      "final_message": "I've updated the view to show...",
      "steps": [
        {
          "iteration": 1,
          "tool": "ng_set_view",
          "args": {"center": {"x": 100, "y": 200, "z": 300}},
          "result": {"ok": true},
          "duration_ms": 45
        },
        {
          "iteration": 2,
          "tool": "ng_set_lut",
          "args": {"layer": "CH_405", "vmin": 0, "vmax": 255},
          "result": {"ok": true},
          "duration_ms": 12
        }
      ]
    }
  ]
}
```

### Available Debug Endpoints

- `GET /debug/tool_trace?n=10` - Get last N tool traces
- `GET /debug/state` - Get current CURRENT_STATE
- `GET /debug/memory` - Get DataMemory statistics

---

## ⚡ Performance Characteristics

### Token Usage Per Request

| Component | Tokens | Notes |
|-----------|--------|-------|
| System Prompt | ~2000 | Static, cached by OpenAI |
| Context (NG state) | ~500 | Dynamic, changes with state |
| Context (Data files) | ~200-500 | Grows with uploaded files |
| User message | ~20-200 | Varies by query length |
| Tool call | ~50-150 | Function name + args |
| Tool result | ~100-500 | Metadata only |
| Assistant response | ~50-300 | Final answer |

**Average request**: 3K-6K tokens
**With long history (10 turns)**: 8K-12K tokens

### Latency Breakdown

```
Total Response Time: ~2-5 seconds

• Network (frontend → backend): ~50ms
• Agent loop iteration:
  - LLM API call: ~1-2s (depends on response length)
  - Tool execution: ~50-500ms (depends on tool)
  - State serialization: ~10ms
• Network (backend → frontend): ~50ms
• Frontend rendering: ~100-300ms
```

### Optimization Strategies

1. **Tool Result Truncation**: Send metadata, not full data
2. **Streaming**: Show responses as they're generated
3. **Context Caching**: OpenAI caches system prompt
4. **Parallel Tool Calls**: Multiple tools in one iteration
5. **Smart State Updates**: Only serialize on mutations

---

## 🎓 Example Conversations

### Example 1: Simple Query

```
User: "Show me 10 cells"

Prompt Size: 2.5K tokens
  → Tool: data_query(limit=10)
Prompt Size: 4K tokens
  → Response: "Here are 10 cells..."
  
Total: 1 iteration, 1 tool call
```

### Example 2: Complex Analysis

```
User: "Plot mean volume per cluster for cells with elongation > 2"

Prompt Size: 2.5K tokens
  → Tool 1: data_list_files()
Prompt Size: 4K tokens
  → Tool 2: data_plot(
      expression="filter(pl.col('elongation') > 2).group_by('cluster').agg(pl.mean('volume'))",
      plot_type="bar"
    )
Prompt Size: 5.5K tokens
  → Response: "Here's a bar plot showing..."
  
Total: 2 iterations, 2 tool calls
```

### Example 3: Error & Recovery

```
User: "Plot invalid_col vs volume"

Prompt Size: 2.5K tokens
  → Tool: data_plot() → Error: "Column 'invalid_col' not found"
Prompt Size: 4K tokens
  → Tool: data_info() → Get valid columns
Prompt Size: 5.5K tokens
  → Response: "The column 'invalid_col' doesn't exist. Available columns are..."
  
Total: 2 iterations, 2 tool calls (1 error)
```

---

## 🔮 Future Enhancements

### Planned Features

1. **Conversation Checkpointing**
   - Save/load conversation state
   - Resume sessions across restarts

2. **Smart Context Pruning**
   - Summarize old turns
   - Keep important tool calls
   - Sliding window with embeddings

3. **Tool Result Caching**
   - Cache expensive queries
   - Deduplicate identical calls
   - TTL-based invalidation

4. **Multi-Agent Collaboration**
   - Specialized agents (query, viz, analysis)
   - Agent routing based on intent
   - Collaborative responses

5. **Enhanced Memory**
   - Vector database for semantic search
   - User preference learning
   - Session-aware responses

---

## 📚 Key Takeaways

✅ **Agent uses iterative loop** with max 10 iterations
✅ **Prompt grows** with conversation history + tool results
✅ **System prompt** provides comprehensive guidelines (~2K tokens)
✅ **Context includes** NG state + available data (~500-1K tokens)
✅ **Tool results** truncated to metadata (no large payloads to LLM)
✅ **History preserved** across turns for coherent conversation
✅ **Streaming support** for real-time user feedback
✅ **Observability** via tool traces and debug endpoints

